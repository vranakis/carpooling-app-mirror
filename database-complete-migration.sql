-- Complete Database Migration for Carpooling App
-- Run this in Supabase SQL Editor to ensure all required columns exist

-- Enable PostGIS extension first (required for spatial data types)
CREATE EXTENSION IF NOT EXISTS postgis;

-- Add missing estimated_arrival_time column to rides table (CRITICAL FIX)
ALTER TABLE rides ADD COLUMN IF NOT EXISTS estimated_arrival_time TIMESTAMP WITH TIME ZONE;

-- Add Google Maps integration columns to rides table
...
-- Create table for route segments (for advanced route matching)
