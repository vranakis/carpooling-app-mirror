...
export default async function HomePage() {
  ...
  const { data: rides } = await supabase.from("rides").select("*").limit(3).order("departure_time", { ascending: true })

  return (
    <div className="container mx-auto px-4 py-8">
      {/* Welcome Section */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Welcome back, {profile?.first_name || user.email}!</h1>
        <p className="text-gray-600">Ready to share a ride and reduce your carbon footprint?</p>
      </div>

      {/* Debug Info - Remove in production */}
      <div className="mb-8 p-4 bg-blue-50 rounded-lg">
        <h3 className="font-semibold text-blue-900 mb-2">Session Debug Info:</h3>
        <p className="text-sm text-blue-800">User ID: {user.id}</p>
        <p className="text-sm text-blue-800">Email: {user.email}</p>
        <p className="text-sm text-blue-800">
          Profile: {profile?.first_name} {profile?.last_name}
        </p>
        <p className="text-sm text-blue-800">Session Active: ✅</p>
      </div>
                ...
                <div key={ride.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm">{new Date(ride.departure_time).toLocaleDateString()}</span>
                    </div>
                    <div className="flex items-center space-x-2">
